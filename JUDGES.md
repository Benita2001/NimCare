...
Payment apps move money without meaning. Messaging apps carry meaning without money. NimCare connects the two: a CareDrop is neither a tip (no expectation of service) nor a bounty (the gift is never conditional on completing the prompt — it's delivered and verified before any response happens). The recipient responding and the note revealing is an emotional payoff layered on top of an already-completed gift, not a gate in front of it. Target users are people who already know each other and want their transfers to carry weight — not strangers, not a discovery mechanism. Repeat value comes from the growing shared Memory and the ongoing Loop relationship, not a one-time transaction.

## Design & UX — 10

Warm/premium visual direction (terracotta accent, warm neutral background, generous rounded corners — see `DESIGN.md`), mobile-first (verified with no horizontal overflow at 375×812 via the browser tool), a clear primary action ("Send a CareDrop"), wallet onboarding explained in plain language before the permission prompt, and a deliberate reveal moment for the sealed note (fade-up animation, respects `prefers-reduced-motion`). The product leads with "Send more than money," not blockchain terminology — a judge should understand the pitch in under 60 seconds (see the one-sentence pitch in `SUBMISSION.md`).
