...
| Functionality/reliability (45 pts) | Scoring | Critical | Critical Demo Path completes without crash | All P0 + hardening fixes | 23 automated tests, live prod smoke test | `MEMORY.md` | VERIFIED (server-side); on-device UI walkthrough UNTESTED — see `DEVICE_TESTING.md` |
...
| Operational | A persistent database is available in this environment (no cloud DB credentials known yet) | "Persistence" claim would be false | Check environment/tooling in Phase 1 | Phase 1 | Use local SQLite/Postgres for dev; document production deployment as a human action | **RESOLVED** — real Neon Postgres provisioned via the Vercel Marketplace, used in both dev and production; SQLite fully migrated away from, not just "for dev" |
